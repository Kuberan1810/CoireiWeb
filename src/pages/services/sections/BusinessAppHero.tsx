...
interface BusinessAppHeroProps {
    onTypingComplete?: () => void;
}

const BusinessAppHero = ({ onTypingComplete }: BusinessAppHeroProps) => {
    const fullText = "Custom Business Solutions for Business Growth";
                ...
                <div className='mb-10'>
                    ...
                    <div className="flex items-center justify-center pointer-events-none">
                        <h1 className="text-4xl md:text-5xl lg:text-6xl font-medium tracking-tight text-[#262626] text-center leading-tight max-w-5xl">
                            {(() => {
                                const part1 = "Custom Business Solutions";
                                if (displayedText.length <= part1.length) {
                                    return <span className="text-[#F67300]">{displayedText}</span>;
                                } else {
                                    const remaining = displayedText.substring(part1.length);
                                    return (
                                        <>
                                            <span className="text-[#F67300]">{part1}</span>
                                            <span>{remaining}</span>
                                        </>
                                    );
                                }
